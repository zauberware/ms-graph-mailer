...
# Version management script for MS Graph Mailer Gem
...
class VersionBumper
  ...
  VERSION_FILE = 'lib/ms/graph/mailer/version.rb'
  CHANGELOG_FILE = 'CHANGELOG.md'

  def initialize(bump_type)
    @bump_type = bump_type.to_s.downcase
    @current_version = MsGraphMailer::VERSION
    validate_bump_type!
  end
  ...
  def update_changelog(new_version)
    ...
    if unreleased_index
      ...
      if unreleased_link_index
        ...
        lines[unreleased_link_index] = "[Unreleased]: https://github.com/zauberware/ms-graph-mailer/compare/v#{new_version}...HEAD\n"
        # Add new version link
        lines.insert(unreleased_link_index + 1, "[#{new_version}]: https://github.com/zauberware/ms-graph-mailer/releases/tag/v#{new_version}\n")
      end
    end
